...
file.list <- list.files(path = "~/Github/StringBase/corpus/original", 
...
for (file in file.list) 
{
  ...
  # distinguish between ancient languages and other files
  if(subcorpus == "ancient"){
    # remove tabs and parentheses
    textfile <- str_replace_all(textfile, c("\\\t" = "", "\\(" = "", "\\)" = "", "\\]" = "", "\\[" = ""))
    #textfile <- gsubfn(".", list("\t" = "", "(" = "", ")" = "", "]" = "",
    #                          "[" = ""), textfile)
    # split textfile into strings of utf-8 character which represent
    # ancient signs. We here use hypens and white spaces as delimiters.
    chars <- unlist(strsplit(textfile, "\\-|\\s|\\{|\\}"))
    chars <- chars[1:n] # use only maximally n units
    chars <- chars[!is.na(chars)] # remove NAs for vectors which are already shorter
    # than n
    chars <- chars[chars != ""] # remove empty strings from character vector
...
```{r}
write.csv(estimations.df, "/home/chris/Github/StringBase/code/Tables/output_stringBase.csv", row.names = F)
```
